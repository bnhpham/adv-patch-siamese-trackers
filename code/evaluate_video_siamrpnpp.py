...
from models.siamrpnpp.siamrpnpp import build_siamrpnpp_tracker, cfg
from utils import paste_rotated_patch, load_patch, make_video, paste_patch_on_frame


# SiamRPN++ analogue of the evaluate_video_siamrpn.py scripts' update_with_patch():
# Paste the patch into the cropped/resized search image before inference, then reproduces pysot's SiamRPNTracker.track() update.
#
# The code is adapted from PySOT's SiamRPNTracker.track() (Copyright (c) SenseTime, Apache License 2.0, see models/siamrpnpp/PYSOT_LICENSE)
# and reuses the PySOT tracker's own crop/decode helpers (get_subwindow, anchors, window, _convert_score/_convert_bbox, _bbox_clip).
...
def main():
    ...
    parser.add_argument("--backbone", choices=["resnet50", "alexnet", "mobilenet"], default="resnet50",
                         help="Selects the --weights/--config defaults below when those aren't given explicitly.")
    parser.add_argument("--weights", default=None)
    parser.add_argument("--config", default=None)
    parser.add_argument("--patch", default=None)
    parser.add_argument("--out_dir", default="results/siamrpnpp/video/person-7")
    ...
    # --weights/--config, when not given explicitly, are resolved from --backbone. This keeps the two in
    # sync by default - the "wrong config for these weights" bug looks like a mistracked/garbage bbox.
    BACKBONE_DEFAULTS = {
        "resnet50": ("models/siamrpnpp/model_resnet50.pth", "models/siamrpnpp/config_resnet50.yaml"),
        "alexnet": ("models/siamrpnpp/model_alexnet.pth", "models/siamrpnpp/config_alexnet.yaml"),
        "mobilenet": ("models/siamrpnpp/model_mobilenet.pth", "models/siamrpnpp/config_mobilenet.yaml"),
    }
    default_weights, default_config = BACKBONE_DEFAULTS[args.backbone]
    weights = args.weights or default_weights
    config = args.config or default_config

    # Initialize tracker
    tracker = build_siamrpnpp_tracker(net_path=weights, config_path=config)
...
if __name__ == "__main__":
    ...
    # Transferability test against SiamRPN++ (siamrpn_r50_l234_dwxcorr (RestNet-50)):
    # python .\evaluate_video_siamrpnpp.py --init_x 557 --init_y 330 --init_w 83 --init_h 202 --patch patch/patch_best.pt --start 1 --end 1000 --patch_x 20 --patch_y 20 --backbone resnet50

    # New patch (crafted using SiamRPN++)
    # python .\evaluate_video_siamrpnpp.py --init_x 557 --init_y 330 --init_w 83 --init_h 202 --patch patch_siamrpnpp/patch_best.pt --start 1 --end 1000 --patch_x 20 --patch_y 20